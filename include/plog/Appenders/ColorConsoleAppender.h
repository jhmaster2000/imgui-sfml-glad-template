    ...
    protected:
        void setColor(Severity severity)
        {
            if (this->m_isatty)
            {
                ...
                case info:
                    SetConsoleTextAttribute(this->m_outputHandle, static_cast<WORD>(foreground::kBlue | foreground::kIntensity | (m_originalAttr & 0xf0))); // blue
                    break;

                case debug:
                    SetConsoleTextAttribute(this->m_outputHandle, static_cast<WORD>(foreground::kGreen | foreground::kIntensity | (m_originalAttr & 0xf0))); // green
                    break;
